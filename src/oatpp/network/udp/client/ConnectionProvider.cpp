...
void ConnectionProvider::ConnectionInvalidator::invalidate(const std::shared_ptr<data::stream::IOStream>& connection) {
  ...
  auto c = std::static_pointer_cast<network::udp::Connection>(connection);
...
provider::ResourceHandle<data::stream::IOStream> ConnectionProvider::get() {
  ...
  if (res != 0) {
#if defined(WIN32) || defined(_WIN32)
    throw std::runtime_error("[oatpp::network::udp::client::ConnectionProvider::getConnection()]. "
                             "Error. Call to getaddrinfo() failed with code " + std::to_string(res));
#else
    std::string errorString = "[oatpp::network::udp::client::ConnectionProvider::getConnection()]. Error. Call to getaddrinfo() failed: ";
	  throw std::runtime_error(errorString.append(gai_strerror(res)));
#endif
  }

  if (result == nullptr) {
    throw std::runtime_error("[oatpp::network::udp::client::ConnectionProvider::getConnection()]. Error. Call to getaddrinfo() returned no results.");
  }
  ...
  if(currResult == nullptr) {
    throw std::runtime_error("[oatpp::network::udp::client::ConnectionProvider::getConnection()]: Error. Can't connect: " +
  ...
  if(ret < 0) {
    OATPP_LOGD("[oatpp::network::udp::client::ConnectionProvider::getConnection()]", "Warning. Failed to set %s for socket", "SO_NOSIGPIPE")
  }
#endif

  return provider::ResourceHandle<data::stream::IOStream>(
      std::make_shared<oatpp::network::udp::Connection>(clientHandle),
      m_invalidator
  );
  ...
  public:
    ...
    Action act() override {
      ...
      if (res != 0) {
        return error<async::Error>(
          "[oatpp::network::udp::client::ConnectionProvider::getConnectionAsync()]. Error. Call to getaddrinfo() failed.");
      }
      ...
      if (m_result == nullptr) {
        return error<async::Error>(
          "[oatpp::network::udp::client::ConnectionProvider::getConnectionAsync()]. Error. Call to getaddrinfo() returned no results.");
      }
    ...
    Action iterateAddrInfoResults() {
      ...
      if(m_currentResult != nullptr) {
        ...
        if(ret < 0) {
          OATPP_LOGD("[oatpp::network::udp::client::ConnectionProvider::getConnectionAsync()]", "Warning. Failed to set %s for socket", "SO_NOSIGPIPE")
        }
#endif
      ...
      return error<Error>("[oatpp::network::udp::client::ConnectionProvider::getConnectionAsync()]: Error. Can't connect.");
    ...
    Action doConnect() {
      ...
      if(res == 0 || error == WSAEISCONN) {
        return _return(provider::ResourceHandle<data::stream::IOStream>(
                std::make_shared<oatpp::network::udp::Connection>(m_clientHandle),
      ...
      if(error == WSAEWOULDBLOCK || error == WSAEINPROGRESS) {
        ...
        return ioRepeat(m_clientHandle, oatpp::async::Action::IOEventType::IO_EVENT_WRITE);
         ...
         return AbstractCoroutine::error(new async::Error(
                  "[oatpp::network::udp::client::ConnectionProvider::doConnect()]: Error. The parameter m_clientHandle is a listening socket."));
      }
