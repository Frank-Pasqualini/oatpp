...
oatpp::v_io_handle ConnectionProvider::instantiateServer(){
  ...
  if (iResult != 0) {
    OATPP_LOGE("[oatpp::network::udp::server::ConnectionProvider::instantiateServer()]", "Error. Call to getaddrinfo() failed with result=%d", iResult)
    throw std::runtime_error("[oatpp::network::udp::server::ConnectionProvider::instantiateServer()]: Error. Call to getaddrinfo() failed.");
  }
    ...
    if (serverHandle != INVALID_SOCKET) {
      ...
      if (hints.ai_family == AF_UNSPEC || hints.ai_family == AF_INET6) {
        if (setsockopt(serverHandle, IPPROTO_IPV6, IPV6_V6ONLY, (char*)&no, sizeof( int ) ) != 0 ) {
          ...
          char buf[buflen];
          OATPP_LOGW("[oatpp::network::udp::server::ConnectionProvider::instantiateServer()]",
  ...
  if (currResult == nullptr) {
    OATPP_LOGE("[oatpp::network::udp::server::ConnectionProvider::instantiateServer()]",
               "Error. Couldn't bind. WSAGetLastError=%ld", WSAGetLastError())
    throw std::runtime_error("[oatpp::network::udp::server::ConnectionProvider::instantiateServer()]: "
                             "Error. Couldn't bind ");
  }

  u_long flags = 1;
  if(NO_ERROR != ioctlsocket(serverHandle, FIONBIO, &flags)) {
    throw std::runtime_error("[oatpp::network::udp::server::ConnectionProvider::instantiateServer()]: Error. Call to ioctlsocket failed.");
  }
...
oatpp::v_io_handle ConnectionProvider::instantiateServer(){
  ...
  if (ret != 0) {
    OATPP_LOGE("[oatpp::network::udp::server::ConnectionProvider::instantiateServer()]", "Error. Call to getaddrinfo() failed with result=%d: %s", ret, strerror(errno))
    throw std::runtime_error("[oatpp::network::udp::server::ConnectionProvider::instantiateServer()]: Error. Call to getaddrinfo() failed.");
  }
...
void ConnectionProvider::prepareConnectionHandle(oatpp::v_io_handle handle) {
  ...
  if(ret < 0) {
    OATPP_LOGD("[oatpp::network::udp::server::ConnectionProvider::prepareConnectionHandle()]", "Warning. Failed to set %s for socket", "SO_NOSIGPIPE")
  }
#endif
...
provider::ResourceHandle<data::stream::IOStream> ConnectionProvider::getExtendedConnection() {
  ...
  if (clientAddress.ss_family == AF_INET) {
    ...
    OATPP_LOGE("[oatpp::network::udp::server::ConnectionProvider::getExtendedConnection()]", "Error. Unknown address family.")
    return nullptr;
