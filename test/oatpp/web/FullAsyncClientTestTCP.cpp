...
#include "FullAsyncClientTestTCP.hpp"
...
public:
  ...
  Action handleError(Error* error) override {
    if(error->is<oatpp::AsyncIOError>()) {
      ...
      OATPP_LOGE("[FullAsyncClientTestTCP::ClientCoroutine_getRootAsync::handleError()]", "AsyncIOError. %s, %ld", e->what(), e->getCode())
    } else {
      OATPP_LOGE("[FullAsyncClientTestTCP::ClientCoroutine_getRootAsync::handleError()]", "Error. %s", error->what())
    }
    OATPP_ASSERT(!"Error")
  }
...
public:
  ...
  Action handleError(Error* error) override {
    if(error->is<oatpp::AsyncIOError>()) {
      auto e = static_cast<oatpp::AsyncIOError*>(error);
      OATPP_LOGE("[FullAsyncClientTestTCP::ClientCoroutine_postBodyAsync::handleError()]", "AsyncIOError. %s, %ld", e->what(), e->getCode())
    } else {
      OATPP_LOGE("[FullAsyncClientTestTCP::ClientCoroutine_postBodyAsync::handleError()]", "Error. %s", error->what())
    }
    OATPP_ASSERT(!"Error")
  }
...
public:
  ...
  Action handleError(Error* error) override {
    if(error) {
      if(error->is<oatpp::AsyncIOError>()) {
        auto e = static_cast<oatpp::AsyncIOError*>(error);
        OATPP_LOGE("[FullAsyncClientTestTCP::ClientCoroutine_echoBodyAsync::handleError()]", "AsyncIOError. %s, %ld", e->what(), e->getCode())
      } else {
        OATPP_LOGE("[FullAsyncClientTestTCP::ClientCoroutine_echoBodyAsync::handleError()]", "Error. %s", error->what())
      }
    }
    OATPP_ASSERT(!"Error")
  }
...
void FullAsyncClientTestTCP::onRun() {
